...
- **rules/analyzers** apply the heuristics shipped in `apiseclint/core.py`.
- **score** ranks by severity.
- **MCP server** (`apiseclint mcp`) exposes `scan` for Cognis.Studio agents.
